...
class EventDecompiler {
    ...
    ArrayList<String> parseEventString(byte[] input, ScriptDecompiler decompiler, int line) {
        lineNumber = line;
        for (int x = 0; x < input.length; x++) {
            ...
            if (input[x] == EVENT_BYTE) {
                if (parameters.size() > 0 && !decompiler.getFileName().startsWith("A002_OP1")) {
                    if (parameters.get(0).getKey().startsWith("raw")) {
                        int index = parameters.get(0).getValue();
                        savedPoints.replace(index, decompiled.size());
                        decompiled.add(getWhiteSpace() + parameters.get(0).getKey());
                        parameters.remove(0);
                    }
                }
    ...
    private SimpleEntry<String, Integer> parseEventCommand(byte[] input, int index, ScriptDecompiler decompiler) {
    ...
    private static SimpleEntry<String, Integer> parseIntegerValue(byte[] input, int index) {
        ...
        if (command == 0x01 || command == 0x07 || command == 0x19)
            ...
            advance = 4;
        if (command == 0x01)
            result += "call(0x" + Long.toHexString(input[index + 1] & 0xFF).toUpperCase() + ")";
        else if (command == 0x07)
            result += "storein(0x" + Long.toHexString(input[index + 1] & 0xFF).toUpperCase() + ")";
        else if (command == 0x19)
            result += "byte(0x" + Long.toHexString(input[index + 1] & 0xFF).toUpperCase() + ")";
        else if (command == 0x1A)
            result += "short(0x" + Long.toHexString(ScriptUtils.shortFromByteArray(input, index + 1)).toUpperCase() + ")";
        else if (command == 0x1B)
            result += "int(0x" + Long.toHexString(Integer.toUnsignedLong(ByteUtils.toInt(input, index + 1))).toUpperCase() + ")";
        else if (command == 0x1F)
            result += "coord(" + ScriptUtils.readCoordBytes(input, index + 1) + ")";
        return new SimpleEntry<>(result, advance);
    }

    private static SimpleEntry<String, Integer> parseString(byte[] input, int index, ScriptDecompiler decompiler) {
    ...
    private SimpleEntry<String, Integer> parseKnownRaw(byte[] input, int index, ScriptDecompiler decompiler) {
        String result = "";
        int advance = 0;
        byte command = input[index];
        if (command == 0x46) {
    ...
    private SimpleEntry<String, Integer> parseRaw(byte[] input, int index) {
        ...
        for (int x = 0; x < raw.size(); x++) {
            result.append("0x").append(Integer.toHexString(raw.get(x) & 0xFF).toUpperCase());
            if (x < raw.size() - 1)
                result.append(",");
            else
                result.append(")");
